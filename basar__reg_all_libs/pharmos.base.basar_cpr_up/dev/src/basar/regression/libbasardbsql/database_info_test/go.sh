...
rm -f database_info_test go.out
...
cat > "$LOG_CONFIG_DIR/database_info_test.cfg" << 'EOF'
...
echo "=========================================" 2>&1 | tee go.out
echo "  Building database_info_test" 2>&1 | tee -a go.out
...
# Path: regression/libbasardbsql/database_info_test/ 
...
./database_info_test 2>&1 | tee -a go.out
exit_code=$?
