...
# Template generator for creating new Basar regression tests (AIX Version)
# Usage: ./new_test_aix.sh <library_name> <test_name>
# Example: ./new_test_aix.sh libbasarcmnutil my_test
...
# Create Makefile - AIX VERSION (no duplicate variables after EOF)
echo "Creating makefile..."
cat > makefile << 'EOF'
##############################################################
#                                                            #
#   Basar Regression Test: TEST_NAME_PLACEHOLDER            #
#                                                            #
##############################################################

TARGET_NAME = TARGET_NAME_PLACEHOLDER
...
sed -i "s/TEST_NAME_PLACEHOLDER/${TEST_NAME}/g" makefile
sed -i "s/TARGET_NAME_PLACEHOLDER/${TEST_NAME}/g" makefile
...
# Regression test runner script for TEST_NAME_PLACEHOLDER
...
rm -f TEST_NAME_PLACEHOLDER go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building TEST_NAME_PLACEHOLDER" 2>&1 | tee -a go.out
...
if [ ! -f TEST_NAME_PLACEHOLDER ]; then
    echo "" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd TEST_NAME_PLACEHOLDER 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./TEST_NAME_PLACEHOLDER 2>&1 | tee -a go.out
exit_code=$?
...
# Replace placeholders in go.sh
sed -i "s/TEST_NAME_PLACEHOLDER/${TEST_NAME}/g" go.sh
chmod +x go.sh
