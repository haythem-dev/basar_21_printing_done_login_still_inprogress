...
# Regression test runner script for user_credentials_test
...
cat > "$LOG_CONFIG_DIR/user_credentials_test.cfg" << 'EOF'
...
rm -f user_credentials_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building: user_credentials_test" 2>&1 | tee -a go.out
...
if [ ! -f user_credentials_test ]; then
    echo "" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd user_credentials_test 2>&1 | grep -E "libbasar|liblog4cplus|libBml|libssl" | tee -a go.out

# Run the test
echo "" 2>&1 | tee -a go.out
echo "=========================================" 2>&1 | tee -a go.out
echo "  Running: user_credentials_test" 2>&1 | tee -a go.out
echo "=========================================" 2>&1 | tee -a go.out
echo "" 2>&1 | tee -a go.out

./user_credentials_test 2>&1 | tee -a go.out
exit_code=$?
