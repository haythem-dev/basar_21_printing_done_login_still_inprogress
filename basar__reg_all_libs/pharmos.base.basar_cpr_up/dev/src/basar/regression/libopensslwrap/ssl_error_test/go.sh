...
# Regression test runner for ssl_error_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/ssl_error_test.cfg" << 'EOF'
...
rm -f ssl_error_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building ssl_error_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd ssl_error_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./ssl_error_test 2>&1 | tee -a go.out
exit_code=$?
