...
# Regression test runner for ssl_context_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/ssl_context_test.cfg" << 'EOF'
...
rm -f ssl_context_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building ssl_context_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd ssl_context_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./ssl_context_test 2>&1 | tee -a go.out
exit_code=$?
