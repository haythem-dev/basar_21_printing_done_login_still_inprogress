...
# Regression test runner for ssl_method_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/ssl_method_test.cfg" << 'EOF'
...
rm -f ssl_method_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building ssl_method_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd ssl_method_test 2>&1 | grep -E "libbasar|liblog4cplus|libssl|libcrypto" | tee -a go.out
...
./ssl_method_test 2>&1 | tee -a go.out
exit_code=$?
