...
# Regression test runner for property_table_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/property_table_test.cfg" << 'EOF'
...
rm -f property_table_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building property_table_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd property_table_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./property_table_test 2>&1 | tee -a go.out
exit_code=$?
