...
# Regression test runner for column_operations_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/column_operations_test.cfg" << 'EOF'
...
rm -f column_operations_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building column_operations_test" 2>&1 | tee -a go.out
...
if [ ! -f column_operations_test ]; then
    echo "" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd column_operations_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./column_operations_test 2>&1 | tee -a go.out
exit_code=$?
