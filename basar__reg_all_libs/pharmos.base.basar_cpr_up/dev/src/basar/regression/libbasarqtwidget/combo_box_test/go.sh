...
# Regression test runner for combo_box_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/combo_box_test.cfg" << 'EOF'
...
rm -f combo_box_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building combo_box_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd combo_box_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./combo_box_test 2>&1 | tee -a go.out
exit_code=$?
