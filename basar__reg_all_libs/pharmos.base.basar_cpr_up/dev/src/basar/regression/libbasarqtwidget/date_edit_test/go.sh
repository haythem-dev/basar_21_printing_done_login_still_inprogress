...
# Regression test runner for date_edit_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/date_edit_test.cfg" << 'EOF'
...
rm -f date_edit_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building date_edit_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd date_edit_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./date_edit_test 2>&1 | tee -a go.out
exit_code=$?
