...
# Regression test runner for table_widget_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/table_widget_test.cfg" << 'EOF'
...
rm -f table_widget_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building table_widget_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd table_widget_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./table_widget_test 2>&1 | tee -a go.out
exit_code=$?
