...
# Regression test runner for transformer_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/transformer_test.cfg" << 'EOF'
...
rm -f transformer_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building transformer_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd transformer_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./transformer_test 2>&1 | tee -a go.out
exit_code=$?
