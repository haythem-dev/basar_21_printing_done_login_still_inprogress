...
# Regression test runner for guitie_widget_types_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/widget_types_test.cfg" << 'EOF'
...
rm -f widget_types_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building guitie_widget_types_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd widget_types_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./widget_types_test 2>&1 | tee -a go.out
exit_code=$?
