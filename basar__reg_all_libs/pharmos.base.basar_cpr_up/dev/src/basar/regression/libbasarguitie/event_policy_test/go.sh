...
# Regression test runner for event_policy_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/event_policy_test.cfg" << 'EOF'
...
rm -f event_policy_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building event_policy_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd event_policy_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./event_policy_test 2>&1 | tee -a go.out
exit_code=$?
