...
# Regression test runner for matcher_policy_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/matcher_policy_test.cfg" << 'EOF'
...
rm -f matcher_policy_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building matcher_policy_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd matcher_policy_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./matcher_policy_test 2>&1 | tee -a go.out
exit_code=$?
