...
# Regression test runner for exception_handler_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/exception_handler_test.cfg" << 'EOF'
...
rm -f exception_handler_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building exception_handler_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd exception_handler_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./exception_handler_test 2>&1 | tee -a go.out
exit_code=$?
