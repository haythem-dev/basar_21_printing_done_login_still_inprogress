...
cat > "$LOG_CONFIG_DIR/infrastructure_test.cfg" << 'EOF'
...
echo "=========================================" 2>&1 | tee -a go.out
ldd infrastructure_test 2>&1 | tee -a go.out

# Run the test
echo "" 2>&1 | tee -a go.out
./infrastructure_test 2>&1 | tee -a go.out
exit_code=$?
