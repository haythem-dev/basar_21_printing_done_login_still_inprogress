...
cat > "$LOG_CONFIG_DIR/threading_test.cfg" << 'EOF'
...
echo "=========================================" 2>&1 | tee -a go.out
ldd threading_test 2>&1 | tee -a go.out

# Run the test
echo "" 2>&1 | tee -a go.out
./threading_test 2>&1 | tee -a go.out
exit_code=$?
