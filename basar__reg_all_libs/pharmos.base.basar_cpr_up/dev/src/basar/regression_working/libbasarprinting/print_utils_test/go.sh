...
# Regression test runner for print_utils_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/print_utils_test.cfg" << 'EOF'
...
rm -f print_utils_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building print_utils_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd print_utils_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./print_utils_test 2>&1 | tee -a go.out
exit_code=$?
