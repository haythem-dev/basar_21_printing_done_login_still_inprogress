...
# Regression test runner for raw_print_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/raw_print_test.cfg" << 'EOF'
...
rm -f raw_print_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building raw_print_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd raw_print_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./raw_print_test 2>&1 | tee -a go.out
exit_code=$?
