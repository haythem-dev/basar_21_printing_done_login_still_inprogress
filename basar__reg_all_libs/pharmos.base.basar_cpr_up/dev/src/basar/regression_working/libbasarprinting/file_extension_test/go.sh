...
# Regression test runner for file_extension_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/file_extension_test.cfg" << 'EOF'
...
rm -f file_extension_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building file_extension_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd file_extension_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./file_extension_test 2>&1 | tee -a go.out
exit_code=$?
