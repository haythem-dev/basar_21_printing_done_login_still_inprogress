...
# Regression test runner for interpreter_test
# AIX 7.2 Compatible
...
cat > "$LOG_CONFIG_DIR/interpreter_test.cfg" << 'EOF'
...
if [ -n "$INFORMIXDIR" ]; then
    export LIBPATH="../../../library/lib/debug:../../../../../ext/log4cplus/lib/debug:../../../../../ext/boost/lib:../../../../../ext/libssh2/lib/debug:../../../../../ext/ghostscript/lib:../../../../../ext/zlib/lib/debug:../../../../../ext/openssl/lib:$INFORMIXDIR/lib/esql:$INFORMIXDIR/lib:$LIBPATH"
else
...
rm -f interpreter_test go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building interpreter_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd interpreter_test 2>&1 | grep -E "libbasar|liblog4cplus" | tee -a go.out
...
./interpreter_test 2>&1 | tee -a go.out
exit_code=$?
