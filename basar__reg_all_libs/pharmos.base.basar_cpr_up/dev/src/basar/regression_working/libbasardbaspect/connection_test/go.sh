...
cat > "$LOG_CONFIG_DIR/connection_test.cfg" << 'EOF'
...
rm -f connection_test *.o go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building connection_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd connection_test 2>&1 | tee -a go.out
...
./connection_test 2>&1 | tee -a go.out
exit_code=$?
