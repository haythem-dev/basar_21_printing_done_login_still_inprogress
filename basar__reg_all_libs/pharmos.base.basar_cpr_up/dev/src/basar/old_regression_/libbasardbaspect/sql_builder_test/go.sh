...
cat > "$LOG_CONFIG_DIR/sql_builder_test.cfg" << 'EOF'
...
rm -f sql_builder_test *.o go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building sql_builder_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd sql_builder_test 2>&1 | tee -a go.out
...
./sql_builder_test 2>&1 | tee -a go.out
exit_code=$?
