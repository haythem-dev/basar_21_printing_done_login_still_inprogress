...
cat > "$LOG_CONFIG_DIR/caching_policy_test.cfg" << 'EOF'
...
rm -f caching_policy_test *.o go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building caching_policy_test" 2>&1 | tee -a go.out
...
g++ -g -std=c++17 -maix64 -pthread \
    ...
    -I../../../../../ext/boost/include \
    -o caching_policy_test main.cpp \
    -L../../../library/lib/debug \
...
echo "=========================================" 2>&1 | tee -a go.out
ldd caching_policy_test 2>&1 | tee -a go.out
...
./caching_policy_test 2>&1 | tee -a go.out
exit_code=$?
