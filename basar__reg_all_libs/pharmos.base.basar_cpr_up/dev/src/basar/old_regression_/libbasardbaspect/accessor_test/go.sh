...
cat > "$LOG_CONFIG_DIR/accessor_test.cfg" << 'EOF'
...
# Add CICS library (required by libbasardbsql.so)
# Add Informix libraries required by libbasardbsql.so
...
rm -f accessor_test *.o go.out

# Build the test
echo "=========================================" 2>&1 | tee go.out
echo "  Building accessor_test" 2>&1 | tee -a go.out
...
echo "=========================================" 2>&1 | tee -a go.out
ldd accessor_test 2>&1 | tee -a go.out
...
./accessor_test 2>&1 | tee -a go.out
exit_code=$?
