...
TEST_FIXTURE(Fixture, QtWidget_max)
{
	...
	QDateTimeEdit dt_ed;
	QLocale locale;
	QString dateStr = locale.toString(d_ed.minimumDate(), QLocale::ShortFormat);
	QString timeStr = locale.toString(t_ed.minimumTime(), QLocale::ShortFormat);
	QString dateTimeStr = locale.toString(dt_ed.minimumDateTime(), QLocale::ShortFormat);
	CHECK_EQUAL("14.09.52", dateStr.toLocal8Bit().constData());
	dateStr = locale.toString(d_ed.maximumDate(), QLocale::ShortFormat);
	CHECK_EQUAL("31.12.99", dateStr.toLocal8Bit().constData());
	CHECK_EQUAL("00:00", timeStr.toLocal8Bit().constData());
	timeStr = locale.toString(t_ed.maximumTime(), QLocale::ShortFormat);
	CHECK_EQUAL("23:59", timeStr.toLocal8Bit().constData());
	CHECK_EQUAL("14.09.52 00:00", dateTimeStr.toLocal8Bit().constData());
	dateTimeStr = locale.toString(dt_ed.maximumDateTime(), QLocale::ShortFormat);
	CHECK_EQUAL("31.12.99 23:59", dateTimeStr.toLocal8Bit().constData());
